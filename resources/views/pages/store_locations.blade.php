...
<script>
  ...
  const svg = `<?xml version="1.0" encoding="UTF-8"?><svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 32' width='36' height='48'><path fill='#b91c1c' d='M12 2C8 2 5 5 5 9c0 6.6 7 13 7 13s7-6.4 7-13c0-4-3-7-7-7z'/><circle cx='12' cy='9' r='3' fill='#fff'/></svg>`;
      ...
      document.addEventListener('click', function(e){
          ...
          if(!isNaN(lat) && !isNaN(lng)){
            try{
              if(typeof map !== 'undefined' && window.google && window.google.maps){
                // clear existing markers then add focused marker
        ...
        fetch(apiUrl).then(r => r.ok ? r.json() : Promise.reject(r)).then(list => {
          if(Array.isArray(list) && list.length){
            // prefer first matching location
            const loc = list[0];
          ...
          // Finally, if nothing else works, try to load markers for category/tag
